...
function cancelarVenda(){	
    ...
    $.post('/admin/venda/pedidosremovervenda', {
    	ped: $("input[name=ped]").val(), 
...
function gerarCancelamento(){	
	...
	$("#subtitulojanelager").html("Cancelando Venda - Enviando a sefaz...");
...
function gerarAjustestoque(){	
	...
	$("#subtitulojanelager").html("Cancelando Venda - Ajustando estoque...");
	
	xmlHttp=GetXmlHttpObject();
	var url="/admin/venda/pedidosremovervenda/venda/"+ped;
...
/*--------- Baixar financeiro -----------------*/
function removeFinanceiro(){	
	
	$("#subtitulojanelager").html("Cancelando Venda - Removendo financeiro...");
	
	xmlHttp=GetXmlHttpObject();
	var url="/admin/administracao/removerecebimentopornfe/rmfin/"+idnfe;
	xmlHttp.onreadystatechange=estadoRetornofinanceiro;
	xmlHttp.open("GET",url,true);
	xmlHttp.send(null);
}

function estadoRetornofinanceiro(){	
	if (xmlHttp.readyState==4){		
		var texto = xmlHttp.responseText;
		texto = texto.replace(/^\s+|\s+$/g,"");
		
		if(texto == "sucessormfinanceiro"){					
	    	$('#barraprogresso').animate( {
	  		  width: '340px',
	  		  opacity: 1,
	  		  left: '0px'
	  			...
	  			$("#subtitulojanelager").css({color:'#006d6e'}).html('Cancelado com sucesso! &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; <a class="botaoconfirma" href="javascript:void(0)" onclick="window.location=\'/admin/venda/pedidosvenda/idped/'+ped+'\'" > &nbsp; Ok &nbsp; </a>');
	  		} );
